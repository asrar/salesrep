...
class OrderProductWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    String _addOnText = '';
    orderDetails.addOns==null?'':orderDetails.addOns.forEach((addOn) {
      _addOnText = _addOnText + '${(_addOnText.isEmpty) ? '' : ',  '}${addOn.name} (${addOn.quantity})';
    });

    String _variationText = '';
    if(orderDetails.variation==null?false:orderDetails.variation.length > 0) {
      ...
      Row(children: [
        ClipRRect(
          ...
          child: FadeInImage.assetNetwork(
            ...
            image: '${Get.find<SplashController>().configModel.baseUrls.productImageUrl}/'
                '${orderDetails.foodDetails==null?"":orderDetails.foodDetails.image}',
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(
                orderDetails.foodDetails==null?"":orderDetails.foodDetails.name,
            ...
            Row(children: [
              Text(
                PriceConverter.convertPrice(orderDetails.price==null?0.0:orderDetails.price - orderDetails.discountOnFood==null?0.0:orderDetails.discountOnFood),
                style: robotoMedium,
              ),
              SizedBox(width: 5),
